...
//More Information:  http://en.wikipedia.org/wiki/Blowfish_cipher
...
BLOWFISH::BLOWFISH(string hexKey)
{
	IvSet = false;
...
byte* BLOWFISH::Crypt_ECB(byte* data, int length, int* newlength, void (BLOWFISH::*CryptBlock)(byte*, int), bool decrypt)
{
	...
	if (decrypt) //если мы расшифровываем, мы должны найти, где заканчиваются данные.
	{
...
byte* BLOWFISH::Crypt_CBC(byte* data, int length, int* newlength, void (BLOWFISH::*CryptBlock)(byte*, int), bool decrypt)
{
	...
	if (!decrypt)
	{
		//padData оставляет 8-байтовый блок в начале, поэтому мы можем сохранить IV
		for (int i = 0; i < 8; i++)
			pData[i] = IV[i];
	}
	else
	...
	byte nextIV[8];
	for (int i = 8; i < *newlength; i += 8) //шифрование данных
	{
		if (!decrypt)
		{
			for (int k = 0; k < 8; k++)
				pData[k + i] ^= pData[k + i - 8]; //предыдущий блок содержит вектор инициализации the initialization vector
		}
		else
		{
			for (int k = 0; k < 8; k++)
	...
	if (decrypt) //если мы расшифровываем, мы должны найти, где заканчиваются данные, и удалить IV
	{
